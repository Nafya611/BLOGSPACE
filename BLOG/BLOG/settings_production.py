...
if DATABASE_URL and "postgres" in DATABASE_URL and "sslmode=" not in DATABASE_URL:
    if "?" in DATABASE_URL:
...
print(
    f"DATABASE_URL value: {repr(DATABASE_URL[:50])}..."
    if DATABASE_URL
    else "DATABASE_URL is empty"
)
...
if DATABASE_URL and DATABASE_URL.strip():
    try:
        ...
        try:
            ...
            print(
                f"Constructed DATABASE_URL: postgresql://{db_user}:***@{db_host}:{db_port}/{db_name}"
            )

            DATABASES = {"default": dj_database_url.parse(constructed_url)}
            ...
            print(
                f"Using fallback database config with HOST: {os.environ.get('DB_HOST', 'localhost')}"
            )
else:
    ...
    try:
        ...
        print(
            f"Database components: HOST={db_host}, PORT={db_port}, DB={db_name}, USER={db_user}"
        )

        if (
            db_password != "blogpass123" or db_host != "localhost"
        ):  # Only if we have real database credentials
            constructed_url = (
                f"postgresql://{db_user}:{db_password}@{db_host}:{db_port}/{db_name}"
            )
            print(
        ...
        print(
            f"Using final fallback database config with HOST: {os.environ.get('DB_HOST', 'localhost')}"
        )
